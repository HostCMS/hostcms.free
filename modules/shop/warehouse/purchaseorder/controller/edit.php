				...
				if (!is_null($oShop_Item))
				{
					...
					$itemTable .= '
						<tr id="' . $oShop_Warehouse_Purchaseorder_Item->id . '" data-item-id="' . $oShop_Item->id . '">
							...
							<td><a class="delete-associated-item" onclick="mainFormLocker.unlock(); res = confirm(\'' . Core::_('Shop_Warehouse_Purchaseorder.delete_dialog') . '\'); if (res) { var next = $(this).parents(\'tr\').next(); $(this).parents(\'tr\').remove(); $.recountIndexes(next); ' . $onclick . ' } return res;"><i class="fa-solid fa-circle-xmark darkorange"></i></a></td>
						</tr>
					';
				}
			}
