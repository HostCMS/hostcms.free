...
class Core_Mail_Imap extends Core_Servant_Properties
{
	...
	public function execute()
	{
		...
		foreach ($aMailIds as $i)
		{
			...
			$fetchheader = array();
			foreach ($aImap_fetchheader as $value)
			{
	...
	protected function _parseMessage($i)
	{
		...
		// Получаем структуру в виде массива
		$aStructureParts = $this->_structure2array($structure);

		// Рекурсивно обрабатываем каждую часть верхнего уровня
		foreach ($aStructureParts as $index => $part)
		{
			$partNumber = (string)($index + 1); // нумерация с 1
			$this->_processPart($this->_stream, $i, $partNumber, $part);
		}

		// Инициализация пустых значений по умолчанию
		$this->_aMessages[$i]['body'] = '';
		$this->_aMessages[$i]['subtype'] = '';

		// Приоритет: HTML → PLAIN → TEXT → первая текстовая часть (type == 0)
		...
		elseif (isset($this->_aMessages[$i]['multipart']['TEXT']))
		{
			$this->_aMessages[$i]['subtype'] = 'TEXT';
			$this->_aMessages[$i]['body'] = $this->_aMessages[$i]['multipart']['TEXT']['body'];
		}
		else
		{
			// Ищем любую часть с типом text (type == 0)
			foreach ($this->_aMessages[$i]['multipart'] as $subtype => $part)
			{
				if (isset($part['type']) && $part['type'] == 0)
				{
					// text type
					$this->_aMessages[$i]['subtype'] = strtoupper($subtype);
					$this->_aMessages[$i]['body'] = $part['body'];
					break;
				}
			}

			// Если не нашли текстовую часть, берем первую доступную
			/*if (empty($this->_aMessages[$i]['body']) && !empty($this->_aMessages[$i]['multipart']))
			{
				$firstPart = reset($this->_aMessages[$i]['multipart']);
				$firstSubtype = key($this->_aMessages[$i]['multipart']);

				$this->_aMessages[$i]['subtype'] = strtoupper($firstSubtype);
				$this->_aMessages[$i]['body'] = $firstPart['body'];
			}*/
		}
	...
	/**
	 * Рекурсивная обработка части сообщения
	 *
	 * @param IMAP\Connection $stream  IMAP-поток
	 * @param int $i                   Номер письма
	 * @param string $number           Номер части (например, "1.2.3")
	 * @param array $part              Структура части (из _structure2array)
	 */
	protected function _processPart($stream, $i, $number, $aPart)
	{
		/*
		[0] = "text"
		[1] = "multipart"
		...
		[7] = "other"
		*/
		$type = isset($aPart['type']) ? $aPart['type'] : 0;

		// Если это multipart, обрабатываем дочерние части
		if ($type == 1)
		{
			if (isset($aPart['parts']) && is_array($aPart['parts']))
			{
				foreach ($aPart['parts'] as $childIndex => $childPart)
				{
					/*
					...
					*/
					...
					$this->_processPart($stream, $i, $childNumber, $childPart);
				}
			}
		}
		else
		{
			// Получаем тело части
			$body = imap_fetchbody($stream, $i, $number);

			$encoding = isset($aPart['encoding']) ? $aPart['encoding'] : 0;
			$body = $this->_bodyDecode($body, $encoding);

			// $aPart encoding
			$charset = isset($aPart['charset']) && $aPart['charset'] != ''
				? $aPart['charset']
				: NULL;

			is_null($charset) && $charset = isset($aPart['parameters']['charset'])
				&& $aPart['parameters']['charset'] != ''
					? $aPart['parameters']['charset']
					: NULL;

			if (!is_null($charset) && strtolower($charset) != 'utf-8')
			{
			...
			$subtype = isset($aPart['id'])
				? trim($aPart['id'], '<>')
				: (isset($aPart['subtype']) ? $aPart['subtype'] : 'unknown');

			$bText = in_array(strtolower($subtype), array('plain', 'html', 'text'));

			$disposition = isset($aPart['disposition']) ? strtolower($aPart['disposition']) : '';
			...
			switch ($type)
			{
				...
				case 0: // text
				case 2: // message
					// Сохраняем как тело письма только если это текстовый тип
					if ($bText)
					{
						$this->_aMessages[$i]['subtype'] = strtoupper($subtype);
					...
					{
						// Нетекстовые части сохраняем как вложения
						$this->_saveAsAttachment($i, $aPart, $body);
					}
				break;
				default:
					// Все остальное - вложения
			...
			{
				$filename = '';
				if (isset($aPart['parameters']['name']))
				{
					$filename = $aPart['parameters']['name'];
				}
				elseif (isset($aPart['dparameters']['filename']))
				{
					$filename = $aPart['dparameters']['filename'];
				}

				if ($filename !== '')
				{
					// Декодируем имя файла
					//$aPart['name'] = iconv_mime_decode($filename, 0, 'UTF-8');
					$aPart['name'] = $this->decodeMimeheader($filename);
				}
			}

			$this->_aMessages[$i]['multipart'][$subtype] = $aPart;
		}
	}

	/**
	 * Сохранение части как вложения
	 *
	 * @param int $i Порядковый номер письма
	 * @param array $aStructurePart Структура части
	 * @param string $body Тело части
	 * @param int $n Счетчик вложений
	 */
	protected function _saveAsAttachment($i, $aStructurePart, $body)
	{
		$aTmp = array(
			'type' => Core_Array::get($aStructurePart, 'type', 0),
			'subtype' => Core_Array::get($aStructurePart, 'subtype', ''),
			'body' => $body
		);
		...
		{
			$filename = $aStructurePart['parameters']['name'];
		}
		elseif (isset($aStructurePart['dparameters']['filename']))
		{
			$filename = $aStructurePart['dparameters']['filename'];
		}

		// Декодируем имя файла если нужно
		$aTmp['name'] = !empty($filename)
			// ? mb_decode_mimeheader($filename)
			// ? iconv_mime_decode($filename, 0, 'UTF-8')
			? $this->decodeMimeheader($filename)
			: '';

		$this->_aMessages[$i]['attachments'][] = $aTmp;
	}
	 ...
	 * @return string
	 */
	public function decodeMimeheader($str)
	{
		if (preg_match('/=\?([^?]+)\?([BQ])\?([^?]+)\?=/', $str, $matches))
		{
			...
			{
				// Пытаемся преобразовать из заявленной кодировки $charset в UTF-8
	...
	protected function _bodyDecode($text, $encoding = 0)
	{
		...
		switch ($encoding)
		{
			...
			case 3: // Base64
				...
				if ($decoded === FALSE)
				{
					// Если не валидный base64, пробуем через imap_base64
