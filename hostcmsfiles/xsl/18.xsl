...
<!DOCTYPE xsl:stylesheet SYSTEM "lang://18">
		...
		<xsl:if test="text != '' or subject != ''">
			...
			<div class="comment" id="comment{@id}">
				...
				<p class="tags">
					...
					<span class="red" onclick="$('.comment_reply').hide('slow');$('#cr_{@id}').toggle('slow')">&labelAnswer;</span></xsl:if>

					<span class="red"><a href="{/informationsystem/informationsystem_item/url}#comment{@id}" title="&labelReviewLink;">#</a></span>
				</p>
			</div>
		...
		<div class="comment">
			...
			<form action="{/informationsystem/informationsystem_item/url}" name="comment_form_0{$id}" method="post">
				...
				<div class="row">
					<div class="caption"></div>
					<div class="field">
						<input id="submit_email{$id}" type="submit" name="add_comment" value="&labelSend;" class="button" />
					</div>
				</div>
			</form>
		</div>
