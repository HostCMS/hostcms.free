	...
	<xsl:template match="shop_item">
		...
		<div>
			...
			<xsl:if test="price != 0">
				...
				<xsl:if test="marking != ''">
					<div class="shop_property">
						&labelMarking; <span hostcms:id="{@id}" hostcms:field="marking" hostcms:entity="shop_item"><xsl:value-of select="marking" /></span>
					</div>
				</xsl:if>

				<xsl:if test="shop_producer/node()">
					<div class="shop_property">
						&labelProducer; <span><xsl:value-of select="shop_producer/name" /></span>
					</div>
				</xsl:if>
				...
				<xsl:if test="weight != 0">
					<div class="shop_property">
						&labelWeight; <span hostcms:id="{@id}" hostcms:field="weight" hostcms:entity="shop_item"><xsl:value-of select="weight" /></span><xsl:text> </xsl:text><span><xsl:value-of select="/shop/shop_measure/name" /></span>
					</div>
				</xsl:if>
				...
				<xsl:if test="rest &gt; 0 and type != 1">
					<div class="shop_property">
						&labelRest; <span><xsl:value-of select="rest - reserved" /><xsl:text> </xsl:text><xsl:value-of select="shop_measure/name" /></span><xsl:if test="reserved &gt; 0"> (&labelReserve; <span><xsl:value-of select="reserved" /><xsl:text> </xsl:text><xsl:value-of select="shop_measure/name" /></span>)</xsl:if>
					</div>
				</xsl:if>
				...
				<xsl:if test="length != 0 or width != 0 or height != 0">
					<div class="shop_property">
						&labelSizes; <span><xsl:value-of select="length" /><xsl:text> </xsl:text><xsl:value-of select="/shop/size_measure/name" />
							<xsl:text> × </xsl:text>
			...
			<xsl:if test="count(modifications/shop_item) &gt; 0">
				<p class="h2">
					&labelModifiations; <xsl:value-of select="name" />
				</p>
				<ul class="shop_list">
			...
			<xsl:if test="count(associated/shop_item) &gt; 0">
				<p class="h2">
					&labelTying; <xsl:value-of select="name" />
				</p>
				<ul class="shop_list">
			...
			<xsl:if test="/shop/show_comments/node() and /shop/show_comments = 1">
				...
				<xsl:if test="count(comment)">
					<p class="h1">
						<a name="comments"></a>&labelReviews;
					</p>
				...
				<xsl:template match="comment">
					...
					<xsl:if test="text != '' or subject != ''">
						...
						<div class="comment" id="comment{@id}">
							<xsl:if test="subject != ''">
								<div class="subject" hostcms:id="{@id}" hostcms:field="subject" hostcms:entity="comment">
									<xsl:value-of select="subject" />
								</div>
							</xsl:if>

							<div hostcms:id="{@id}" hostcms:field="text" hostcms:entity="comment" hostcms:type="wysiwyg">
